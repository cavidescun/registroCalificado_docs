...
Procedimiento especializado para procesar observaciones y actualizaciones de la Condición 9 (Infraestructura) en el sistema BPM. Maneja observaciones JSON, actualiza campos editables específicos de infraestructura y consolida la traza de observaciones en el maestro unificado.

#### Diagrama de flujo

```mermaid
flowchart TD
    A[Inicio: sp_MaestroObsSubprocC9] --> B[Recibe parámetros C9 + observaciones]
    ...
    J --> K[Configurar Condición 9 e IdFormulario 22]
    K --> L[Procesar JSON observaciones con OPENJSON]
    L --> M[INSERT en TM_Observaciones]
    M --> N[UPDATE campos editables C9]
...
EXEC [CUN].[sp_MaestroObsSubprocC9] '000000002088'
EXEC [CUN].[sp_MaestroObsSubprocC9] '000000002098'

--=================================================================================================================================================================\*/

CREATE Procedure [CUN].[sp_MaestroObsSubprocC9] @pNumCaso VARCHAR(250), @pNumCasoPadre varchar(250), @pAccionPorCondicion varchar(max),
@pC9Edit_numeroEspacios VARCHAR(MAX),
@pC9Edit_sedesResumen VARCHAR(MAX),
@pC9Edit_proyeccionAsignacionDeRecursos VARCHAR(MAX),
...
Set @Condicion = 'Condición 9 - Infraestructura' --C9_RevPDF
Set @IdFormulario = 22
...
,observacionesH
,1 estado
,'CUN.sp_MaestroObsSubprocC9: '+@pNumCaso+convert(varchar(200), getdate(), 121) Auditoria
,@IdFormulario
...
--print 'update'
UPDATE TMU
SET TMU.numeroEspacios = isnull(@pC9Edit_numeroEspacios ,TMU.numeroEspacios )
,TMU.sedesResumen = isnull(@pC9Edit_sedesResumen ,TMU.sedesResumen )
,TMU.proyeccionAsignacionDeRecursos = isnull(@pC9Edit_proyeccionAsignacionDeRecursos ,TMU.proyeccionAsignacionDeRecursos )
,ObservaInfraestructuraC9 = case when @pAccionPorCondicion = 2 then 1 else 0 end  
...
- Actualización campos C9: Modifica campos específicos de infraestructura
...
- CUN.TM_MaestroUnificado: Campos AccionPorCondicion, numeroEspacios, sedesResumen, proyeccionAsignacionDeRecursos, ObservaInfraestructuraC9, trazaDeObservaciones
