...
Procedimiento especializado para procesar observaciones y actualizaciones de la Condición 4 (Organización académica) en el sistema BPM. Maneja observaciones JSON y actualiza 13 campos editables específicos relacionados con estructura curricular, competencias, metodologías y organización académica.

#### Diagrama de flujo

```mermaid
flowchart TD
    A[Inicio: sp_MaestroObsSubprocC4] --> B[Recibe parámetros C4 + observaciones]
    ...
    J --> K[Configurar Condición 4 e IdFormulario 17]
    K --> L[Procesar JSON observaciones con OPENJSON]
    L --> M[INSERT en TM_Observaciones]
    M --> N[UPDATE 13 campos editables C4]
...
EXEC [CUN].[sp_MaestroObsSubprocC4] '000000002088'
EXEC [CUN].[sp_MaestroObsSubprocC4] '000000002098'

--=================================================================================================================================================================\*/

CREATE Procedure [CUN].[sp_MaestroObsSubprocC4] @pNumCaso VARCHAR(250), @pNumCasoPadre varchar(250), @pAccionPorCondicion varchar(max),
@pC4Edit_cursoDisciplinar varchar(max),  
 @pC4Edit_editableRae varchar(max),  
 @pC4Edit_cursosTransversales varchar(max),  
 @pC4Edit_cursosElectivos varchar(max),  
 @pC4Edit_cursosComplementarios varchar(max),  
 @pC4Edit_cursosDeProfundizacion varchar(max),  
 @pC4Edit_enfoqueComplementario varchar(max),  
 @pC4Edit_actividadAcademica varchar(max),  
 @pC4Edit_competenciaHabilidades varchar(max),  
 @pC4Edit_horasSincronicasAsincronicas varchar(max),  
 @pC4Edit_ajusteDeAsignaturas varchar(max),  
 @pC4Edit_planTransicion varchar(max),  
 @pC4Edit_componente varchar(max),  
...
Set @Condicion = 'Condición 4 - Organización académica' --C4_RevPDF
Set @IdFormulario = 17
...
,observacionesH
,1 estado
,'CUN.sp_MaestroObsSubprocC4: '+@pNumCaso+convert(varchar(200), getdate(), 121) Auditoria
,@IdFormulario
...
---
    ...
    --Proceso de actualización campos editables condicion 1
    UPDATE TMU
       SET TMU.cursoDisciplinar              = isnull(@pC4Edit_cursoDisciplinar                 ,TMU.cursoDisciplinar             )
          ,TMU.editableRae                   = isnull(@pC4Edit_editableRae                      ,TMU.editableRae                  )
          ,TMU.cursosTransversales           = isnull(@pC4Edit_cursosTransversales              ,TMU.cursosTransversales          )
          ,TMU.cursosElectivos               = isnull(@pC4Edit_cursosElectivos                  ,TMU.cursosElectivos              )
          ,TMU.cursosComplementarios         = isnull(@pC4Edit_cursosComplementarios            ,TMU.cursosComplementarios        )
          ,TMU.cursosDeProfundizacion        = isnull(@pC4Edit_cursosDeProfundizacion           ,TMU.cursosDeProfundizacion       )
          ,TMU.enfoqueComplementario         = isnull(@pC4Edit_enfoqueComplementario            ,TMU.enfoqueComplementario        )
          ,TMU.actividadAcademica            = isnull(@pC4Edit_actividadAcademica               ,TMU.actividadAcademica           )
          ,TMU.competenciaHabilidades        = isnull(@pC4Edit_competenciaHabilidades           ,TMU.competenciaHabilidades       )
          ,TMU.horasSincronicasAsincronicas  = isnull(@pC4Edit_horasSincronicasAsincronicas     ,TMU.horasSincronicasAsincronicas )
          ,TMU.ajusteDeAsignaturas           = isnull(@pC4Edit_ajusteDeAsignaturas              ,TMU.ajusteDeAsignaturas          )
          ,TMU.planTransicion                = isnull(@pC4Edit_planTransicion                   ,TMU.planTransicion               )
          ,TMU.componente                    = isnull(@pC4Edit_componente                       ,TMU.componente                   )
          ,TMU.ObservaContenidosCurriC4      = case when  @pAccionPorCondicion = 2 then 1 else 0 end
...
- Actualización campos C4: Modifica 13 campos específicos de organización académica
...
- CUN.TM_MaestroUnificado: Campos AccionPorCondicion, cursoDisciplinar, editableRae, cursosTransversales, cursosElectivos, cursosComplementarios, cursosDeProfundizacion, enfoqueComplementario, actividadAcademica, competenciaHabilidades, horasSincronicasAsincronicas, ajusteDeAsignaturas, planTransicion, componente, ObservaContenidosCurriC4, trazaDeObservaciones
