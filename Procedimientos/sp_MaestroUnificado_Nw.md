...
Versión nueva (Nw) del procedimiento maestro del sistema BPM4US que representa la evolución más avanzada para consolidar datos académicos. Incorpora mejoras significativas en mapeo de datos, campos adicionales de estructura curricular V2, y procesamiento optimizado de tablas JSON, manteniendo la funcionalidad core pero con extensiones para nuevos requerimientos académicos.

#### Diagrama de flujo

```mermaid
flowchart TD
    A[Inicio: sp_MaestroUnificado_Nw] --> B[Parámetros: SNIES, NumCaso, TipoProceso, etc.]
    B --> C[Control inicial y validación en TM_MaestroUnificado]
    C --> D[Búsqueda escuela/director desde rcal.Acta]
    D --> E[Activación campos BPM y mapeo básico]
    E --> F[Mapeo extenso con FN_RemoverCaracterEspecial]
    F --> G[Procesamiento estructura curricular V2]
    G --> H[Generación JSON optimizada para tablas complejas]
    H --> I[Mapeo campos V2 y nuevas etiquetas académicas]
    I --> J[Procesamiento gráficos estadísticos avanzados]
    J --> K[Mapeo inventarios y proyecciones mejoradas]
    K --> L[Campos adicionales de investigación y RAE]
    L --> M[SELECT final del registro consolidado]
    M --> N[Fin]
...
--=================================================================================================================================================================_/
*/
CREATE Procedure [CUN].[sp_MaestroUnificado_Nw]
( @pSNIES varchar(255)
...
A.TR_Semestre_Asignatura_Contenido_V2 = CUN.FN_RemoverCaracterEspecial(B.TR_Semestre_Asignatura_Contenido_V2),
...
WHERE A.NumeroCaso = @pNumCaso
    ...
    SET A.F2_Diferenciadores_Programa_V2 = CUN.FN_RemoverCaracterEspecial(B.F2_Diferenciadores_Programa_V2)
    FROM [CUN].[TM_MaestroUnificado] A
    INNER JOIN [RCAL].tbl_EstructuraCurricular_v2 B
    		ON B.TR_SNIES = A.CodigoSnies
    WHERE A.NumeroCaso = @pNumCaso
    ...
    UPDATE [CUN].[TM_MaestroUnificado]
       SET F5_Grupos_Interinstitucionales_V2  = (SELECT CUN.FN_RemoverCaracterEspecial(TR_SNIES)	 'TR_SNIES'
    									 ,CUN.FN_RemoverCaracterEspecial(GRUPO)						 'GRUPO'
    									 ,CUN.FN_RemoverCaracterEspecial(CATEGORIA_COLCIENCIAS)		 'CATEGORIA_COLCIENCIAS'
    								  FROM [RCAL].[F5_Grupos_Interinstitucionales_V2]
       								 WHERE TR_SNIES = @pSNIES
    								   FOR JSON PATH)
    WHERE NumeroCaso = @pNumCaso;
    ...
    WHERE A.NumeroCaso = @pNumCaso
     ...
     UPDATE [CUN].[TM_MaestroUnificado]
       SET Acceso_CUNdigital  = (SELECT CUN.FN_RemoverCaracterEspecial(TR_SNIES)		 'TR_SNIES'
    									 ,CUN.FN_RemoverCaracterEspecial(Usuario)		 'Usuario'
    ...
    SET F3_RAE_TEC  = (SELECT CUN.FN_RemoverCaracterEspecial(DOMINIO_DE_COMPETENCIA)                                       'Dominio de competencia'
    					   ...
    					   ,CUN.FN_RemoverCaracterEspecial(TR_NumeroActa) 'Numero Acta'
    					   ,CUN.FN_RemoverCaracterEspecial(F_NumeroAcuerdo) 'NumeroAcuerdo'
    				 FROM dbo.F3_RAE_TEC --21-10 Joha: Se actualiza tabla
    				WHERE TR_SNIES = @pSNIES
    				 FOR JSON PATH)
    WHERE NumeroCaso = @pNumCaso;
    ...
    SET F3_RAE_TP  = (SELECT CUN.FN_RemoverCaracterEspecial(DOMINIO_DE_COMPETENCIA)                                         'Dominio de competencia'
    					   ...
    					   ,CUN.FN_RemoverCaracterEspecial(TR_NumeroActa) 'Numero Acta'
    					   ,CUN.FN_RemoverCaracterEspecial(F_NumeroAcuerdo) 'NumeroAcuerdo'
    				 FROM dbo.F3_RAE_TP --21-10 Joha: Se actualiza tabla
    				WHERE TR_SNIES = @pSNIES
    				 FOR JSON PATH)
    WHERE NumeroCaso = @pNumCaso;
    ...
    SET F3_RAE_PROF  = (SELECT CUN.FN_RemoverCaracterEspecial(DOMINIO_DE_COMPETENCIA)                                      'Dominio de competencia'
    					   ...
    					   ,CUN.FN_RemoverCaracterEspecial(TR_NumeroActa) 'Numero Acta'
    					   ,CUN.FN_RemoverCaracterEspecial(F_NumeroAcuerdo) 'NumeroAcuerdo'
    				 FROM dbo.F3_RAE_PROF --21-10 Joha: Se actualiza tabla
    				WHERE TR_SNIES = @pSNIES
    				 FOR JSON PATH)
    WHERE NumeroCaso = @pNumCaso;
    ...
    SET Distribucion_Creditos_Areas  = (SELECT CUN.FN_RemoverCaracterEspecial(TR_SNIES)		'TR_SNIES'
    					   ...
    					   ,CUN.FN_RemoverCaracterEspecial([%_AREA_CONOCIMIENTO])			'%_AREA_CONOCIMIENTO'
    					   ,CUN.FN_RemoverCaracterEspecial(TOTAL_CREDITOS)					'TOTAL_CREDITOS'
    ...
    UPDATE [CUN].[TM_MaestroUnificado]  --
    SET Distribucion_Trabajo_Academico  = (SELECT CUN.FN_RemoverCaracterEspecial(TR_SNIES)		'TR_SNIES'
    					   ,CUN.FN_RemoverCaracterEspecial(Proporcionalidad)				'Proporcionalidad'
    					   ,CUN.FN_RemoverCaracterEspecial(N_Semanas)						'N_Semanas'
    					   ,CUN.FN_RemoverCaracterEspecial(Creditos)						'Creditos'
    ...
    SET tbl_ProfesorExperiencia  = (SELECT CUN.FN_RemoverCaracterEspecial(TR_NombreDocente)		'Nombre del Docente'
    					   ,CUN.FN_RemoverCaracterEspecial(TR_ExpProfesional)					'Experiencia Profesional'
    					   ,CUN.FN_RemoverCaracterEspecial(TR_ExpDocente)						'Experiencia Docente'
    					   ,CUN.FN_RemoverCaracterEspecial(TR_AntiguedadInstAnos)			    'Antiguedad Inst Anos'
    					   --,CUN.FN_RemoverCaracterEspecial(HAD)								'HAD'
    					   --,CUN.FN_RemoverCaracterEspecial(HTI)								'HTI'
    					   --,CUN.FN_RemoverCaracterEspecial(HT)								'HT'
    					   --,CUN.FN_RemoverCaracterEspecial([%_COMPONENTE])					'%_COMPONENTE'
...
UPDATE [CUN].[TM_MaestroUnificado] --
SET Inventario_software = (SELECT CUN.FN_RemoverCaracterEspecial(Asignatura_segun_el_plan_de_estudios_Actividad_Academica) 'Asignatura_segun_el_plan_de_estudios_Actividad_Academica'
,CUN.FN_RemoverCaracterEspecial(Nombre_del_medio_educativo) 'Nombre_del_medio_educativo'
,CUN.FN_RemoverCaracterEspecial(Descripcion_del_medio_educativo) 'Descripcion_del_medio_educativo'
-- ,CUN.FN_RemoverCaracterEspecial(TR_AntiguedadInstAnos) 'Antiguedad Inst Anos'
--,CUN.FN_RemoverCaracterEspecial(HAD) 'HAD'
--,CUN.FN_RemoverCaracterEspecial(HTI) 'HTI'
--,CUN.FN_RemoverCaracterEspecial(HT) 'HT'
...
--,CUN.FN_RemoverCaracterEspecial(TR_NumeroActa) 'Numero Acta'
--,CUN.FN_RemoverCaracterEspecial(F_NumeroAcuerdo) 'NumeroAcuerdo'
FROM RCAL.Inventario_software --21-10 Joha: Se actualiza tabla
WHERE TR_SNIES = @pSNIES
FOR JSON PATH)
WHERE NumeroCaso = @pNumCaso;

UPDATE [CUN].[TM_MaestroUnificado] --
SET Inventario_Recurso_Bibliografico = (SELECT CUN.FN_RemoverCaracterEspecial(Descripcion_Formato) 'Descripcion_Formato'
,CUN.FN_RemoverCaracterEspecial(Periodo_1) 'Periodo_1'
,CUN.FN_RemoverCaracterEspecial(Periodo_2) 'Periodo_2'
,CUN.FN_RemoverCaracterEspecial(Periodo_3) 'Periodo_3'
,CUN.FN_RemoverCaracterEspecial(Periodo_4) 'Periodo_4'
,CUN.FN_RemoverCaracterEspecial(Periodo_5) 'Periodo_5'
,CUN.FN_RemoverCaracterEspecial(Periodo_6) 'Periodo_6'
,CUN.FN_RemoverCaracterEspecial(Periodo_7) 'Periodo_7'
FROM RCAL.Inventario_Recurso_Bibliografico --21-10 Joha: Se actualiza tabla
WHERE TR_SNIES = @pSNIES
FOR JSON PATH)
WHERE NumeroCaso = @pNumCaso;

UPDATE [CUN].[TM_MaestroUnificado] --
SET Proyeccion_Financiera = (SELECT CUN.FN_RemoverCaracterEspecial(Descripcion_Formato) 'Descripcion_Formato'
,CUN.FN_RemoverCaracterEspecial(P_Periodo_1) 'Periodo_1'
,CUN.FN_RemoverCaracterEspecial(P_Periodo_2) 'Periodo_2'
,CUN.FN_RemoverCaracterEspecial(P_Periodo_3) 'Periodo_3'
,CUN.FN_RemoverCaracterEspecial(P_Periodo_4) 'Periodo_4'
,CUN.FN_RemoverCaracterEspecial(P_Periodo_5) 'Periodo_5'
,CUN.FN_RemoverCaracterEspecial(P_Periodo_6) 'Periodo_6'
,CUN.FN_RemoverCaracterEspecial(P_Periodo_7) 'Periodo_7'
...
UPDATE [CUN].[TM_MaestroUnificado] --
SET Proyeccion_Infraestructura_Física_Tecnologica_v2 = (SELECT CUN.INVENTARIO_ACTUAL_2024(Descripcion_Formato) 'INVENTARIO_ACTUAL_2024'
,CUN.FN_RemoverCaracterEspecial(CANTIDAD_2025) 'CANTIDAD_2025'
,CUN.FN_RemoverCaracterEspecial(CANTIDAD_2026) 'CANTIDAD_2026'
,CUN.FN_RemoverCaracterEspecial(CANTIDAD_2027) 'CANTIDAD_2027'
,CUN.FN_RemoverCaracterEspecial(CANTIDAD_2028) 'CANTIDAD_2028'
,CUN.FN_RemoverCaracterEspecial(CANTIDAD_2029) 'CANTIDAD_2029'
,CUN.FN_RemoverCaracterEspecial(CANTIDAD_2030) 'CANTIDAD_2030'
...
- Control y validación mejorados: Inserción optimizada en TM_Control con validaciones
- Mapeo curricular V2: Incluye nuevos campos de estructura curricular versión 2
- Procesamiento JSON avanzado: Estructuras JSON optimizadas para mejor rendimiento
- Campos de investigación extendidos: Mapeo completo de RAE por niveles formativos
- Inventarios detallados: Procesamiento de software, bibliografía y proyecciones financieras

#### Tablas afectadas

##### Principal:

- [CUN].[TM_MaestroUnificado]: Tabla destino con más de 400 campos (extensión V2)
- [CUN].[TM_Control]: Control de ejecuciones

##### Fuentes V2 (120+ tablas):

- Tablas estructura curricular V2
- Tablas RAE específicas por nivel (TEC, TP, PROF)
- Inventarios detallados (software, bibliografía)
- Proyecciones financieras y tecnológicas
- Campos de investigación interinstitucional
